...
def change_routing(route, old_iface, old_gw, new_iface, new_gw, dry_run):
    changed = False
...
def compare_scores(scores, routing, best, data, dry_run):
    for route in data.get('routes'):
        ...
        if best_iface != curr_iface:
            logger.info("Best iface %s is not current %s", best_iface,
                        curr_iface)
            switch_cost = data["routes"].get(route).get("switch_cost", 100)
...
def main():
    ...
    compare_scores(scores, routing, best, data, args.dry_run)
